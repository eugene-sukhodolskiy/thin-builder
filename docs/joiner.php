...
function watch_dir($dir){
	// Normalize path to dir
	if($dir[strlen($dir) - 1] == '/'){
		$dir_parse = explode('/', $dir);
		unset($dir_parse[count($dir_parse) - 1]);
		$dir = implode($dir_parse);
	}

	$files = scandir($dir);
	// remove "." and ".." dirs from list
	unset($files[0]);
	unset($files[1]);
	
	$files_to_watching = [];
	foreach($files as $item){
		if(strpos($item, '.') !== false and strpos($item, '.join.')){
			$files_to_watching[] = $dir . '/' . $item;
			$output_filename = implode('.', explode('.join.', $item));
			system("sudo php joiner.php {$item} {$output_filename} watch > /dev/null &");
		}else{
			watch_dir($dir . '/' . $item);
		}
	}
...
function process(){
	...
	if(isset($argv[3]) and $argv[3] == 'watch'){
		if($output_file == '-' and strpos($input_file, '.join.') !== false){
			watch(__DIR__ . '/' . $input_file, __DIR__ . '/' . implode('.', explode('.join.', $input_file)));
		}else{
			watch(__DIR__ . '/' . $input_file, __DIR__ . '/' . $output_file);
		}
	}elseif(isset($input_file) and $input_file == 'dir'){
		watch_dir(__DIR__ . '/' . $output_file);
	}else{
